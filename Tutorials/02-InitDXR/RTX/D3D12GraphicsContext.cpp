...
IDXGISwapChain3Ptr CppDirectXRayTracing02::D3D12GraphicsContext::createDxgiSwapChain(IDXGIFactory4Ptr pFactory, HWND hwnd, uint32_t width, uint32_t height, DXGI_FORMAT format, ID3D12CommandQueuePtr pCommandQueue)
{
...
ID3D12Device5Ptr CppDirectXRayTracing02::D3D12GraphicsContext::createDevice(IDXGIFactory4Ptr pDxgiFactory)
{
    // Find the HW adapter
...
ID3D12CommandQueuePtr CppDirectXRayTracing02::D3D12GraphicsContext::createCommandQueue(ID3D12Device5Ptr pDevice)
{
...
ID3D12DescriptorHeapPtr CppDirectXRayTracing02::D3D12GraphicsContext::createDescriptorHeap(ID3D12Device5Ptr pDevice, uint32_t count, D3D12_DESCRIPTOR_HEAP_TYPE type, bool shaderVisible)
{
...
D3D12_CPU_DESCRIPTOR_HANDLE CppDirectXRayTracing02::D3D12GraphicsContext::createRTV(ID3D12Device5Ptr pDevice, ID3D12ResourcePtr pResource, ID3D12DescriptorHeapPtr pHeap, uint32_t& usedHeapEntries, DXGI_FORMAT format)
{
...
void CppDirectXRayTracing02::D3D12GraphicsContext::resourceBarrier(ID3D12GraphicsCommandList4Ptr pCmdList, ID3D12ResourcePtr pResource, D3D12_RESOURCE_STATES stateBefore, D3D12_RESOURCE_STATES stateAfter)
{
...
uint64_t CppDirectXRayTracing02::D3D12GraphicsContext::submitCommandList(ID3D12GraphicsCommandList4Ptr pCmdList, ID3D12CommandQueuePtr pCmdQueue, ID3D12FencePtr pFence, uint64_t fenceValue)
{
    pCmdList->Close();
