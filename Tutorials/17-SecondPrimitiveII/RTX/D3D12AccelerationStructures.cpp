...
ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::createBuffer(ID3D12Device5Ptr pDevice, uint64_t size, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES initState, const D3D12_HEAP_PROPERTIES& heapProps)
{
...
CppDirectXRayTracing17II::AccelerationStructureBuffers CppDirectXRayTracing17II::D3D12AccelerationStructures::createPrimitiveBottomLevelAS(ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList)
{
...
CppDirectXRayTracing17II::AccelerationStructureBuffers CppDirectXRayTracing17II::D3D12AccelerationStructures::createCubeBottomLevelAS(ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList)
{
    auto& primitive = mCube;
...
ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::CreateCubeVB(ID3D12Device5Ptr pDevice)
{
    // Vertex buffer
    auto& primitive = mCube;
    int vertexCount = static_cast<int>(primitive.GetVertices().size());
...
ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::CreateCubeIB(ID3D12Device5Ptr pDevice)
{
    auto& primitive = mCube;
    int indexCount = static_cast<int>(primitive.GetIndices().size());
...
ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::CreateSphereVB(ID3D12Device5Ptr pDevice)
{
    // Vertex buffer
...
ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::CreateSphereIB(ID3D12Device5Ptr pDevice)
{
    int indexCount = static_cast<int>(mSphere.GetIndices().size());
...
CppDirectXRayTracing17II::AccelerationStructureBuffers CppDirectXRayTracing17II::D3D12AccelerationStructures::createBottomLevelAS(ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList, ID3D12ResourcePtr vBuffer, ID3D12ResourcePtr iBuffer, int vertexCount, int indexCount)
{
...
CppDirectXRayTracing17II::AccelerationStructureBuffers CppDirectXRayTracing17II::D3D12AccelerationStructures::createTopLevelAS(ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList, ID3D12ResourcePtr pBottomLevelAS[], uint64_t& tlasSize)
{
    ...
    transformation[0] = glm::translate(glm::mat4(1.0), glm::vec3(-2.0f, -6.0f, -2.0f)); // Identity
    transformation[1] = translate(mat4(), vec3(2, 0.3, -2));

    // Initialize the instance desc.
    for (int i = 0; i < kDefaultNumDesc; i++)
    {
...
int CppDirectXRayTracing17II::D3D12AccelerationStructures::GetVertexCount()
{
    return static_cast<int>(mSceneVertices.size());
}

int CppDirectXRayTracing17II::D3D12AccelerationStructures::GetIndexCount()
{
    return static_cast<int>(mSceneIndices.size());
}


ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::GetSphereIndexBuffer()
{
    return mSphereIndexBuffer;
}

ID3D12ResourcePtr CppDirectXRayTracing17II::D3D12AccelerationStructures::GetSphereVertexBuffer()
{
